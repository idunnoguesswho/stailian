...
if (!$id) {
    pageHeader('Character Profiles', 'charbase.php');
    echo flash();
    $chars = $db->query("SELECT * FROM charbase ORDER BY name")->fetchAll();
    ?>
    <h1 class="page-title">Character Profiles</h1>
    <p class="page-sub">Select a character to view their full profile</p>
    <div class="grid-3">
      <?php foreach ($chars as $c): ?>
      <a href="charprofile.php?id=<?= $c['id'] ?>" style="text-decoration:none;">
        <div class="card" style="text-align:center; cursor:pointer; transition: border-color .2s;"
          ...
          <div style="font-family:'Cinzel',serif; font-size:1.1rem; color:var(--gold);"><?= htmlspecialchars($c['name']) ?></div>
          <div style="font-size:.85rem; color:var(--muted); font-style:italic; margin-top:.25rem;"><?= htmlspecialchars($c['chardescription']) ?></div>
        </div>
      </a>
      <?php endforeach; ?>
    </div>
...
// ── FETCH CHARACTER ───────────────────────────────────────────────────────────
$stmt = $db->prepare("SELECT * FROM charbase WHERE id=?");
$stmt->execute([$id]);
$char = $stmt->fetch();
if (!$char) {
    pageHeader('Not Found', '');
    echo '<h1 class="page-title">Character not found.</h1><a class="btn btn-outline" href="charprofile.php">← Back</a>';
    pageFooter(); exit;
}
...
$weaponStmt = $db->prepare("
    ...
    JOIN weapons w ON w.id = ci.weaponid
    WHERE ci.charid = ?
    ORDER BY w.name
");
...
$skillStmt = $db->prepare("
    ...
    FROM charAttributes ca
    JOIN skills s ON s.id = ca.skillid
    WHERE ca.charid = ?
    ORDER BY s.name
");
...
pageHeader('Character Profile — ' . htmlspecialchars($char['name']), 'charbase.php');
?>

<div style="display:flex; align-items:center; gap:1rem; margin-bottom:.25rem;">
  <a class="btn btn-outline btn-sm" href="charprofile.php">← All Characters</a>
  <a class="btn btn-outline btn-sm" href="charbase.php?action=edit&id=<?= $id ?>">Edit Character</a>
</div>
...
<div class="card" style="margin-top:1.25rem; display:flex; align-items:center; gap:2rem; flex-wrap:wrap;">
  ...
  <div style="flex:1;">
    <h1 class="page-title" style="margin-bottom:.1rem;"><?= htmlspecialchars($char['name']) ?></h1>
    <p style="color:var(--muted); font-style:italic; font-size:1rem;"><?= htmlspecialchars($char['chardescription']) ?></p>
    <p style="color:var(--muted); font-size:.8rem; margin-top:.5rem;">Member since <?= $char['created_at'] ?></p>
  </div>
